...
# BJORN AI Installation Script
# This script handles the complete installation of BJORN AI
# Author: infinition
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/bjorn_ai_install_$(date +%Y%m%d_%H%M%S).log"
VERBOSE=false

# Global variables
BJORN_USER="bjorn"
BJORN_PATH="/home/${BJORN_USER}/Bjorn"
BJORN_REPO_URL="https://github.com/infinition/Bjorn.git"
BJORN_REPO_BRANCH="ai"
BJORN_VARIANT="AI"
CURRENT_STEP=0
TOTAL_STEPS=8
...
if [[ "$1" == "--help" ]]; then
    ...
    echo "Installs Bjorn ${BJORN_VARIANT} from branch '${BJORN_REPO_BRANCH}'"
...
# Setup BJORN
setup_bjorn() {
    log "INFO" "Setting up BJORN ${BJORN_VARIANT}..."
    ...
    if [ -d "Bjorn" ]; then
        ...
        log "INFO" "Cloning BJORN ${BJORN_VARIANT} repository (branch: ${BJORN_REPO_BRANCH})"
        git clone --branch "$BJORN_REPO_BRANCH" --single-branch "$BJORN_REPO_URL"
        check_success "Cloned BJORN ${BJORN_VARIANT} repository"
    fi

    cd Bjorn
    if [ -d ".git" ]; then
        log "INFO" "Syncing repository to branch ${BJORN_REPO_BRANCH}..."
        git fetch --all --prune
        check_success "Fetched repository updates"
        if git show-ref --verify --quiet "refs/remotes/origin/${BJORN_REPO_BRANCH}"; then
            git checkout "$BJORN_REPO_BRANCH" 2>/dev/null || git checkout -b "$BJORN_REPO_BRANCH" "origin/${BJORN_REPO_BRANCH}"
            check_success "Checked out branch ${BJORN_REPO_BRANCH}"
            git pull --ff-only origin "$BJORN_REPO_BRANCH"
            check_success "Updated branch ${BJORN_REPO_BRANCH}"
        else
            log "WARNING" "Remote branch '${BJORN_REPO_BRANCH}' not found. Keeping current branch."
        fi
    else
        log "WARNING" "No .git metadata found in $BJORN_PATH. Cannot enforce branch ${BJORN_REPO_BRANCH}."
    fi

    if [ -f "version.txt" ]; then
        INSTALLED_VERSION="$(cat version.txt)"
        log "INFO" "Detected Bjorn ${BJORN_VARIANT} version: ${INSTALLED_VERSION}"
    fi
...
verify_installation() {
    ...
    if ! systemctl is-active --quiet bjorn.service; then
        log "WARNING" "BJORN ${BJORN_VARIANT} service is not running"
    else
        log "SUCCESS" "BJORN ${BJORN_VARIANT} service is running"
    fi
...
clean_exit() {
    ...
    if [ $exit_code -eq 0 ]; then
        log "SUCCESS" "BJORN ${BJORN_VARIANT} installation completed successfully!"
        log "INFO" "Log file available at: $LOG_FILE"
    else
        log "ERROR" "BJORN ${BJORN_VARIANT} installation failed!"
        log "ERROR" "Check the log file for details: $LOG_FILE"
    fi
    exit $exit_code
}
...
main() {
    log "INFO" "Starting BJORN ${BJORN_VARIANT} installation..."
    ...
    echo -e "${BLUE}BJORN ${BJORN_VARIANT} Installation Options:${NC}"
    ...
    case $install_option in
        1)
            ...
            CURRENT_STEP=5; show_progress "Setting up BJORN ${BJORN_VARIANT}"
            setup_bjorn
        ...
        2)
            ...
            read -p "Setup BJORN ${BJORN_VARIANT}? (y/n): " bjorn
    ...
    log "SUCCESS" "BJORN ${BJORN_VARIANT} installation completed!"
    log "INFO" "Please reboot your system to apply all changes."
    echo -e "\n${GREEN}BJORN ${BJORN_VARIANT} installation completed successfully!${NC}"
